...
public class XtendReferenceFinder extends DefaultReferenceFinder implements IReferenceFinder {
  ...
  public void findReferences(final Set<URI> targetURIs, final IResourceDescription resourceDescription, final IAcceptor<IReferenceDescription> acceptor, final IProgressMonitor monitor, final IReferenceFinder.ILocalResourceAccess localResourceAccess) {
    ...
    for (final URI uri : targetURIs) {
      final IUnitOfWork<Boolean, ResourceSet> _function_2 = (ResourceSet it) -> {
        boolean _xblockexpression = false;
        {
          ...
          if (_notEquals) {
            ...
            QualifiedName _qualifiedName = this.nameConverter.toQualifiedName(_lowerCase);
    ...
    final Function1<QualifiedName, QualifiedName> _function_3 = (QualifiedName it) -> {
    ...
    if (_exists_1) {
      URI _uRI = resourceDescription.getURI();
      final IUnitOfWork<Object, ResourceSet> _function_5 = (ResourceSet it) -> {
        URI _uRI_1 = resourceDescription.getURI();
        Resource _resource = it.getResource(_uRI_1, true);
        final IAcceptor<IReferenceDescription> _function_6 = (IReferenceDescription it_1) -> {
          acceptor.accept(it_1);
        };
        this.findLocalReferencesInResource(targetURIs, _resource, _function_6);
        return null;
      };
